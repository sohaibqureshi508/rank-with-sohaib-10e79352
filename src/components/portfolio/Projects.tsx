  ...
  return (
    <section id="projects" className="py-24 sm:py-32 overflow-hidden">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-20">
          <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-primary mb-4">Case Studies</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          {projects.map((project, index) => (
            <a 
              ...
              className="group relative flex flex-col p-10 bg-white border-2 border-border/50 rounded-[3rem] transition-all duration-500 hover:border-primary hover:shadow-[0_30px_60px_rgba(0,0,0,0.08)] hover:-translate-y-2"
            >
              ...
              <h3 className="text-3xl font-black text-foreground mb-4 uppercase tracking-tighter">{project.title}</h3>
              <p className="text-muted-foreground text-lg mb-8 leading-relaxed font-medium">{project.description}</p>
              
              <div className="mt-auto flex flex-wrap gap-3">
                {project.tags.map((tag, i) => (
                  <span key={i} className="text-[10px] font-black uppercase tracking-widest px-4 py-2 bg-zinc-50 text-muted-foreground border border-border rounded-full group-hover:bg-primary/5 group-hover:border-primary/20 group-hover:text-primary transition-colors">
                    {tag}
                  </span>
                ))}
              </div>
